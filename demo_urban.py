...
class DEMO(object):
    ...
    def load_data(self, data_loc):
        ...
        self.data_cube = pkg_data['Urban_norm_cube']
    ...
    def estVAR(self):
        ROI = np.reshape(self.data_cube[210:235, 262:284,:], (25*22, 162))
    ...
    def best_run(self, mrun):
        ...
        #for ki in range(self.p):
            #sad_min_A[:, ki] = sad_min_A[:, ki]-(np.mean(sad_min_A[:, ki])-np.mean(self.groundtruth[:, ki]))
        ...
        sio.savemat(result_path + '/data_out/' + self.name + 'ab', {'sad_ab_min_' + self.name: self.sad_ab_min})
        sio.savemat(result_path + '/data_out/' + self.name + 'em', {'sad_em_min_' + self.name: self.sad_em_min})
...
def run():
    ...
    sio.savemat(result_path + '/data_out/endmember.mat', {'A': endmember})
    sio.savemat(result_path + '/data_out/abundance.mat', {'S': abundance})

    for i in range(0, num_endm):  # 绘制端元对比图
        plt.figure()
        plt.plot(algo[0].groundtruth[:, i]/np.max(algo[0].groundtruth[:, i]), linewidth = '1', label='USGS Library')
        ...
        for idx, k in enumerate(algo):
            ...
            plt.plot(new_endmember[:, i]/np.max(new_endmember[:,i]), linewidth = '0.8', label=k.name)
        ...
        plt.savefig(result_path + '/IMG/' + endmember_names[i] + '_Endmember.png', format='png', dpi=200)
        file.write('![alt text](' + result_path + '/IMG/' + endmember_names[i] + '_Endmember.png)\n\n')
        ...
        plt_s = plot_image_grid(new_abundance, factor=8, nrow=len(algo)+1)
        plt_s.title(endmember_names[i])
        plt_s.tight_layout()
        plt.axis('off')
        plt.xticks([])
        plt.yticks([])
        plt_s.savefig(result_path + '/IMG/' + endmember_names[i] + '_Abundance.png', format='png',
                      dpi=200)
        file.write('![alt text](' + result_path + '/IMG/' + endmember_names[i] + '_Abundance.png)\n\n')


if __name__ == '__main__':

    data_path = "/mnt/Data/SU/data_results/Urban/"
    result_path = "/mnt/Data/SU/data_results/Urban/results_4"  # + str(i+1)
    # pdb.set_trace()
    data_loc = data_path + "raw_data/Urban_norm_cube.mat"
    gt_loc = data_path + "raw_data/groundTruth_Urban_end4/end4_groundTruth.mat"
    abf_true_loc = data_path + "raw_data/groundTruth_Urban_end4/end4_groundTruth.mat"
    ...
    file.write('Maximum number of iterations (KP) : %s \n\n' % iter_KP)  # KP丰度估计和端元更新的迭代次数
    file.write('Maximum number of iterations (BCUN/BCUN0) : %s \n\n' % iterEM)  # BCUN丰度估计和端元更新的迭代次数
    file.write('Maximum number of epoch (BCUN/BCUN0) : %s \n\n' % iters)  # BCUN丰度估计和端元更新的迭代次数
