...
class DEMO(object):
    ...
    def load_data(self, data_loc):
        ...
        self.data_cube = pkg_data['Y_norm1']
    ...
    def estVAR(self):
        ROI = np.reshape(self.data_cube[199:216, 179:189,:],(17*10,188))
    ...
    def load_groundtruth(self, gt_loc):
        ...
        self.groundtruth = pkg_gt['GT_Cuprite'] # (220, 4)
    ...
    def best_run(self, mrun):
        ...
        sio.savemat('./r/data_out_112/' + str(g) + self.name + 'ab', {'sad_ab_min_'+ self.name: self.sad_ab_min})
        sio.savemat('./r/data_out_112/' + str(g) + self.name + 'em', {'sad_em_min_'+ self.name: self.sad_em_min})
...
def run():
    ...
    tab4_sad_stats['Statistics'] = ['_Mean_', '_Std_', '_Time_', '_Mse_']
    tab4_sad_stats.set_index('Statistics', inplace=True)



    tab5_sid_stats = pd.DataFrame()
    tab5_sid_stats['Statistics'] = ['_Mean_', '_Std_', '_Time_', '_Mse_']
    ...
    for l in algo:
        ...
        tab4_sad_stats[l.name] = [np.mean(l.sad_mean), np.mean(l.sad_std), np.mean(l.time_runs),  l.sad_mse]

        tab2_sid[l.name] = l.sid_values_min
        tab5_sid_stats[l.name] = [np.mean(l.sid_mean), np.mean(l.sid_std),np.mean(l.time_runs),  l.sid_mse]
    ...
    snr_w = str(g)
    sio.savemat('./r/data_out_112/endmember' + '_' + snr_w, {'A': endmember})
    sio.savemat('./r/data_out_112/abundance' + '_' + snr_w, {'S': abundance})
    ...
    for i in range(0, 4):  # 绘制端元对比图
        ...
        plt.savefig('./r/IMG_112/SNR=' + snr_w + '_' + endmember_names[i] + '_Endmember.png', format='png', dpi=200)
        file.write('![alt text](./IMG_112/SNR=' + snr_w + '_' + endmember_names[i] + '_Endmember.png)\n\n')
        ...
        plt_s.tight_layout()
        plt_s.savefig('./r/IMG_112/SNR=' + snr_w + '_' + endmember_names[i] + '_Abundance.png', format='png',
                      dpi=200)
        file.write('![alt text](./IMG_112/SNR=' + snr_w + '_' + endmember_names[i] + '_Abundance.png)\n\n')


if __name__ == '__main__':
    data_loc = "./DATA/realHSI/Cuprite/Cuprite_cube_norm.mat" #Samson/Data_Envi/samson_1.mat
    gt_loc = "./DATA/realHSI/Cuprite/GT_Cuprite.mat"
    #abf_true_loc = "./DATA/dip_data/unmixing3/OtherInfo/abf.mat"

    debug = True
    verbose = True
    num_endm = 12    # 混合影像数据的端元数
