...
class MVCNMF:
    ...
    def Z(self, A, mean):
        ...
        if self.U.shape[0] == A.shape[0]:
            ...
            u = self.U_bar
        return self.C + np.dot(np.dot(self.B, u.T), A-np.array([mean]*self.c).T[0])
    ...
    def extract_endmember(self):
        ...
        for iteration in iterator:
            alpha = self.get_alpha(X, A, S, learning_rate, 1, scaling, learning_tolerance)
            A = self.A_new(X, A, S, alpha)
            A_bar = self.augment(A)
            beta = self.get_beta(X_bar, A_bar, S, learning_rate, 1, scaling, learning_tolerance)
    ...
    def get_beta(self, X, A, S, beta, m, scaling, learning_tolerance, max_iter=15):
        ...
        if f_new - f_old < condition or m > max_iter:
            ...
            return self.get_beta(X, A, S, beta_new,m+1, scaling, learning_tolerance, max_iter=max_iter)
