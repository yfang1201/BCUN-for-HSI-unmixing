...
class BCUN(object):
    ...
    def extract_endmember(self):
        ...
        net = get_net(self.nBand, self.NET_TYPE, self.pad, self.upsample_mode, self.k, self.act_fun, self.skip_n33d,
                      self.skip_n33u, self.skip_n11, self.num_scales, self.downsample_mode).type(dtype)
        net = net.float().cuda()
        ...
        nfindr = NFINDR([self.data.cpu().numpy().transpose(), self.nRow, self.nCol, self.nBand, self.npixel, self.k, 3*self.k], self.verbose)
        initA = nfindr.extract_endmember()[0]
        #pdb.set_trace()
        ...
        def closure():
            ...
            #print(var.mean())
            #total_loss = EdisLoss(out_mixed, mixed_input, self.var_noi) ## using fixed var
