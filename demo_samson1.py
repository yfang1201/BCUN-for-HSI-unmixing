...
class DEMO(object):
    ...
    def __init__(self, argin, verbose):
        ...
        self.data = self.convert_2D(self.data_cube)
        self.p = argin[2]
        self.name = argin[3]
        ...
        if argin[3] == 'AEsmm':
             ...
             self.ee = AEsmm([self.data, self.nRow, self.nCol, self.nBand, self.nPixel, self.p, iterEM, iters,
                              ...
                              self.estVAR(), self.groundtruth], self.verbose)   # self.addnoise(self.data, g/2)[0]--->self.data.transpose(1,0)

        if argin[3] == 'AEsmm_mse':
             if (verbose):
                 print('... Selecting AEsmm_mse')
             self.ee = AEsmm([self.data, self.nRow, self.nCol, self.nBand, self.nPixel, self.p, iterEM, iters,
                              INPUT, pad, OPT_OVER, LR_mse, tv_weight, OPTIMIZER, NET_TYPE,
                              self.data.transpose(1, 0),
                              self.estVAR(), self.groundtruth], self.verbose)
        ...
        if argin[3] == 'KPmeans':
            ...
            self.ee = KPmeans([self.data, self.nRow, self.nCol, self.nBand, self.nPixel, self.iter, self.p],self.verbose)
    ...
    def load_data(self, data_loc):
        ...
        self.data_cube = pkg_data['samson_cube']
        self.nRow = self.data_cube.shape[0]
        self.nCol = self.data_cube.shape[1]
        self.nBand = self.data_cube.shape[2]
    ...
    def estVAR(self):
        ROI = np.reshape(self.data_cube[9:48, 4:20,:],(39*16,156))
        var_img = np.var(ROI, axis=0, keepdims=True).squeeze()
        #pdb.set_trace()
        return var_img
    ...
    def load_groundtruth(self, gt_loc):
        ...
        self.groundtruth = pkg_gt['M'] # (220, 4)
    ...
    def best_run(self, mrun):
        ...
        for i in range(mrun):
            ...
            if any( self.name in s for s in ['VCA','PPI', 'NFINDR','MVCNMF']):
                ...
                for ii in range(0, self.nPixel):
                    ...
                    self.S[:, ii] = self.S[:, ii] / sum(self.S[:, ii])
                # pdb.set_trace()
            ...
            ###############################################
                ...
                self.extract_endmember()  # 每种方法各自的端元提取
                self.raw_endmembers = self.ee.endmember
                # for ki in range(self.p):
                #     # self.raw_endmembers[:, ki] = self.raw_endmembers[:, ki] / self.raw_endmembers[:, ki].max()
                #     self.raw_endmembers[:, ki] = np.mean(self.raw_endmembers[:, ki])-np.mean()
                self.S = self.ee.abundance

            elif any( self.name in s for s in ['uDAs']):
                raw_loc = './r/udas_r/'
                a_loc = raw_loc + str(i+1) + '/' + 'A' + '.mat'
                s_loc = raw_loc + str(i+1) + '/' + 'S' + '.mat'
                a_data = sio.loadmat(a_loc)
                s_data = sio.loadmat(s_loc)
                self.raw_endmembers = a_data['A']
                self.S = s_data['S']
                self.S = np.reshape(np.reshape(self.S, [self.p, self.nCol, self.nRow]).transpose(0, 2, 1),
                                    [self.p, self.nPixel])
            ##################################################
        ...
        sio.savemat('./r/data_out_samson/' + self.name + 'ab.mat', {'sad_ab_min_'+ self.name: self.sad_ab_min})
        sio.savemat('./r/data_out_samson/' + self.name + 'em.mat', {'sad_em_min_'+ self.name: self.sad_em_min})

def run():
    endmember_names = ['A', 'B', 'C']
    ...
    algo = [vca, ppi, nfindr, kpmeans, aesmmmse, aesmm]
    ...
    for ll in algo:
        ...
        tab1_sad[ll.name] = ll.sad_values_min
        tab4_sad_stats[ll.name] = [np.mean(ll.sad_mean), np.mean(ll.sad_std), np.mean(ll.time_runs), ll.sad_ssim, ll.sad_mse]

        tab2_sid[ll.name] = ll.sid_values_min
        tab5_sid_stats[ll.name] = [np.mean(ll.sid_mean), np.mean(ll.sid_std), np.mean(ll.time_runs), ll.sid_ssim, ll.sid_mse]

        tab1_aad[ll.name] = ll.aad_values_min
        tab4_aad_stats[ll.name] = [np.mean(ll.aad_mean), np.mean(ll.aad_std), np.mean(ll.time_runs), ll.aad_ssim, ll.aad_mse]

        tab2_aid[ll.name] = ll.aid_values_min
        tab5_aid_stats[ll.name] = [np.mean(ll.aid_mean), np.mean(ll.aid_std), np.mean(ll.time_runs), ll.aid_ssim, ll.aid_mse]
    ...
    sio.savemat('./r/data_out_samson/endmember.mat', {'A': endmember})
    sio.savemat('./r/data_out_samson/abundance.mat', {'S': abundance})

    for i in range(0, num_endm):  # 绘制端元对比图
        plt.figure()
        ...
        plt.savefig('./r/IMG_samson/SNR=' + '_' + endmember_names[i] + '_Endmember.png', format='png', dpi=200)
        file.write('![alt text](./IMG_samson/SNR=' + '_' + endmember_names[i] + '_Endmember.png)\n\n')
        ...
        plt_s = plot_image_grid(new_abundance, factor=8, nrow=3)
        plt_s.title(endmember_names[i])
        plt_s.tight_layout()
        plt_s.savefig('./r/IMG_samson/SNR=' + '_' + endmember_names[i] + '_Abundance.png', format='png',
                      dpi=200)
        file.write('![alt text](./IMG_samson/SNR=' + '_' + endmember_names[i] + '_Abundance.png)\n\n')


if __name__ == '__main__':
    data_loc = "./DATA/realHSI/Samson/Data_Envi/samson_cube.mat"  # Samson/Data_Envi/samson_1.mat
    gt_loc = "./DATA/realHSI/Samson/GroundTruth/end3.mat"
    abf_true_loc = "./DATA/realHSI/Samson/GroundTruth/end3.mat"

    debug = True
    verbose = True
    num_endm = 3   # 混合影像数据的端元数
