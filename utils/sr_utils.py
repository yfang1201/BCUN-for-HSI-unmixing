...
def SAD(A_true, A_new):
        nA = np.matmul(np.transpose(A_true), A_true).diagonal()
        nA_est = np.matmul(np.transpose(A_new.numpy()), A_new.numpy()).diagonal()
        ang_theta = 180/np.pi*np.arccos(np.matmul(np.transpose(A_true),A_new.numpy()).diagonal()/np.sqrt(np.matmul(nA,nA_est)))
        sad = np.sqrt(np.mean(ang_theta*ang_theta))
        return sad

def SID(A_true, A_new):
        sid = KLD(A_true,A_new)+KLD(A_new,A_true)
        return sid
    
def AAD(S_true, S_est):
        nA = np.matmul(np.transpose(S_true), S_est).diagonal()
        nA_est = np.matmul(np.transpose(S_est), S_est).diagonal()
        ang_theta = 180/np.pi*np.arccos(np.matmul(np.transpose(S_true),S_est).diagonal()/np.sqrt(np.matmul(nA,nA_est)))
        aad = np.mean(ang_theta)
        return aad
    
def AID(S_true, S_est):
        aid = KLD(S_true,S_est)+KLD(S_est,S_true)
        return aid
    
    
def MdisLoss(output, target, var):
    P = output.size()[1]
    M = output.size()[2]
    N = output.size()[3]
...
def EdisLoss(output, target, var):
    ...
    output1 = torch.reshape(output,[P,M*N])
    target1 = torch.reshape(target,[P,M*N])
    loss = torch.mean(torch.t(output1 - target1)**2)
    return loss
